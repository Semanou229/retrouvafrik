...
interface RetrouvAfrikLogoProps {
  ...
  height?: number
  showText?: boolean
  variant?: 'icon-only' | 'header' | 'footer'
}
...
export default function RetrouvAfrikLogo({ 
  ...
  // Variant 'icon-only' : seulement la silhouette de l'Afrique
  ...
  // Variant 'header' : Logo pour header - utilise l'image exacte fournie
  if (variant === 'header') {
    return (
      <div className={`flex items-center ${className}`}>
        <Image
          src="/logo-header.png"
          alt="RetrouvAfrik Logo"
          width={width * 3.5}
          height={height}
          ...
          onError={(e) => {
            ...
            console.warn('Logo header image not found, using SVG fallback')
  ...
  // Variant 'footer' : Logo complet avec loupe pour footer - loupe blanche + Afrique orange + RetrouvAfrik
  return (
    <div className={`flex items-center ${className}`}>
      {/* Logo footer : Loupe blanche avec Afrique orange + Texte RetrouvAfrik */}
      <svg
        width={width * 4}
        height={height}
        viewBox="0 0 240 50"
